...
export class BanksController {
  ...
  @Post()
  @UseGuards(JwtAuthGuard)
  async addNewBank(@Req() req: Request, @Body() body: BankBodyDto): Promise<void | HttpException> {
    return await this.bankService.createNewBank(body, req.headers.authorization.slice(7));
  }

  @Get(':id')
  @UseGuards(JwtAuthGuard)
  async getBankById(@Req() req: Request, @Param('id') id: number): Promise<Bank | HttpException> {
    return await this.bankService.findBankById(id, req.headers.authorization.slice(7));
  }

  @Patch(':id')
  @UseGuards(JwtAuthGuard)
  async updateBank(@Req() req: Request, @Param('id') id: number, @Body() body: BankUpdateDto): Promise<Bank | HttpException> {
    return await this.bankService.updateBank(id, body, req.headers.authorization.slice(7));
  }

  @Delete(':id')
  @UseGuards(JwtAuthGuard)
  async deleteBank(@Req() req: Request, @Param('id') id: number): Promise<void | HttpException> {
    return await this.bankService.deleteBank(id, req.headers.authorization.slice(7));
  }

  @Get(':bankId/history')
  @UseGuards(JwtAuthGuard)
  async getAllRecords(@Req() req: Request, @Param('bankId') bankId: number): Promise<Record[] | HttpException> {
    return await this.bankService.findAllRecords(req.headers.authorization.slice(7), bankId);
  }

  @Get(':bankId/history/:id')
  @UseGuards(JwtAuthGuard)
  async getRecord(@Req() req: Request, @Param('bankId') bankId: number, @Param('id') id: number): Promise<Record | HttpException> {
    return await this.bankService.findRecordById(req.headers.authorization.slice(7), bankId, id);
  }

  @Post(':bankId/history')
  @UseGuards(JwtAuthGuard)
  async createRecord(@Req() req: Request, @Param('bankId') bankId: number, @Body() body: RecordBodyDto): Promise<void | HttpException> {
    return await this.bankService.createNewRecord(req.headers.authorization.slice(7), bankId, body);
  }

  @Delete(':bankId/history/:id')
  @UseGuards(JwtAuthGuard)
  async removeRecord(@Req() req: Request, @Param('bankId') bankId: number, @Param('id') id: number): Promise<void | HttpException> {
